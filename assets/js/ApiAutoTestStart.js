   window.onload = function() {
    ...
    // [UPDATED] OPA 030 Bulk Member Body (Password Change)
    const generateBulkMemberBody = () => {
        ...
        // [CHANGE] Password Updated to 'forcs1321!@'
        return [
            {
                "id": bulkId1,
                "password": "forcs1321!@",
                "name": "김테스트",
                "contact": { "tel": "0233334444", "number": "01022223333", "country_number": "+82" },
                "department": "연구소",
                "position": "연구원",
            ...
            },
            {
                ...
                "password": "forcs1321!@",
                "name": "유테스트",
                "contact": { "tel": "0312223333", "number": "01023456789", "country_number": "+82" },
                "department": "경영기획실",
                "position": "사원",
                "role": ["template_manager"]
            }
    ...
    const testList = [
        ...
        // 15. Member Create
        {
            ...
            body: () => {
                ...
                return { "account": { "id": data.memberId, "password": "forcs0421!@", "first_name": "테스터", "external_sso_info": { "uuid": "123", "account_id": "test" } } };
            },
            expectedStatus: 200
        },
        // 16. Member List
        ...
        // --- Company Stamp APIs (OPA 029, 025) ---
        // 23. OPA 029
        {
    ...
    async function runAllTests() {
        ...
        sharedData.companyStampId = null;
        sharedData.bulkMemberIds = [];
        let successCount = 0;
        let failCount = 0;
        ...
        for (let i = 0; i < testList.length; i++) {
            ...
            if (errorMessage === "SKIP_OPA14") {
                ...
                const row = `
                    ...
                    <tr><td colspan="6" class="p-0 border-0"><div id="${rowId}" class="log-box">수신자 정보가 없어 건너뜀</div></td></tr>
                `;
            ...
            if (!errorMessage) {
                try {
                    ...
                    // [UPDATED] Check for logic failure in OPA 030
                    let isPass = (response.status === test.expectedStatus);
                    
                    if (isPass && test.name.includes("OPA 030") && responseJson && responseJson.members) {
                        const failedMembers = responseJson.members.filter(m => m.success === false);
                        if (failedMembers.length > 0) {
                            isPass = false;
                            responseText = "⚠️ [Business Logic Failure] HTTP 200 but 'success': false detected.\n\n" + responseText;
                        }
                    }
            ...
            const row = `
                <tr class="${rowColor}">
                    ...
                    <td class="text-center">
                        <button class="btn btn-sm btn-outline-secondary" onclick="toggleLog('${rowId}')">결과보기</button>
                    </td>
                </tr>
                <tr>
